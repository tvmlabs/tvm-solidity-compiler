...
pub fn print_abi_json_canonically(out: &mut File, value: &serde_json::Value) -> Status {
    let root = value.as_object().ok_or_else(|| anyhow::anyhow!("ABI parsing failed"))?;
    writeln!(out, "{{")?;
    ...
    if let Some(header) = root.get("header") {
        write!(out, "\t\"header\": [")?;
        let array = header.as_array().ok_or_else(|| anyhow::anyhow!("ABI parsing failed"))?;
...
fn print_data(out: &mut File, value: &serde_json::Value) -> Status {
    let json = value.as_array().ok_or_else(|| anyhow::anyhow!("ABI parsing failed"))?;
    for f in 0..json.len() {
        write!(out, "\t\t")?;
...
fn print(out: &mut File, value: &serde_json::Value) -> Status {
    let json = value.as_array().ok_or_else(|| anyhow::anyhow!("ABI parsing failed"))?;
    for f in 0..json.len() {
        let function = json[f].as_object().ok_or_else(|| anyhow::anyhow!("ABI parsing failed"))?;
        writeln!(out, "\t\t{{")?;
        ...
        if let Some(inputs) = function.get("inputs") {
            let array = inputs.as_array().ok_or_else(|| anyhow::anyhow!("ABI parsing failed"))?;
        ...
        if let Some(outputs) = function.get("outputs") {
            let array = outputs.as_array().ok_or_else(|| anyhow::anyhow!("ABI parsing failed"))?;
            print_array(out, array)?;
        }
        writeln!(out, "\t\t\t]")?;
